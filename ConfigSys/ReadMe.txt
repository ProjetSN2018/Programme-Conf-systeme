================================================================================
    BIBLIOTHÈQUE MFC (MICROSOFT FOUNDATION CLASS) : Vue d'ensemble du projet ConfigSys
===============================================================================

L'Assistant Application a créé cette application ConfigSys pour vous. Cette application décrit non seulement les bases de l'utilisation de Microsoft Foundation Classes, mais est également un point de départ pour l'écriture de votre application.

Ce fichier contient un résumé du contenu de chacun des fichiers qui constituent votre application ConfigSys.
...
res\ConfigSys.ico
    Il s'agit d'un fichier icône, qui est utilisé comme icône de l'application. Cette icône est incluse par le fichier de ressources principal ConfigSys.rc.
...
CDlg.h, CDlg.cpp - la boîte de dialogue
    Ces fichiers contiennent votre classe CDlg. Cette classe définit le comportement de la boîte de dialogue principale de votre application. Le modèle de la boîte de dialogue se trouve dans ConfigSys.rc, qui peut être modifié dans Microsoft Visual C++.
...
StdAfx.h, StdAfx.cpp
    Ces fichiers sont utilisés pour générer un fichier d'en-tête précompilé (PCH) ConfigSys.pch et un fichier de type précompilé nommé StdAfx.obj.
...
"Load" enregistre les données dans l'objet mais quand "Save" -> après modif même données que le "Load" car pas SetConf()
Mais si SetConf() avant le "Save", on perd les données non modifiées